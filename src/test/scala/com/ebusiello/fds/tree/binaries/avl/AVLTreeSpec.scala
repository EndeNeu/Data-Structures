...
class AVLTreeSpec extends WordSpecLike with Matchers {
  ...
  trait TestContext {
    val emptyTree = AVLTree.emptyTree[Int]
  }
  ...
  "Binary com.ebusiello.fds.tree" should {
    "correctly insert" in new TestContext {
      ...
      nonEmptyTree.depth should be(2)

      emptyTree.find(0) should be(false)
    }

    "correctly retract (rebalance) after inserting" in new TestContext {
      val t1 = emptyTree.insert(1).insert(2)

      t1.head.value should be(1)
      t1.head.right.value should be(2)
      t1.head.left shouldBe a[EmptyAVLNode[_]]

      // right depth is 2, left 0, now should rebalance the tree.
      val t2 = t1.insert(3)

      t2.head.value should be(2)
      t2.head.right.value should be(3)
      t2.head.left.value should be(1)
    }
    ...
    "correctly remove an element" in new TestContext {
      ...
      n1.head.value should be(40)
      n1.head.right.value should be(50)
      n1.head.left.value should be(35)
      n1.head.left.left.value should be(20)
      n1.head.left.right.value should be(37)
      ...
      t3.head.right.right shouldBe a[EmptyAVLNode[_]]
